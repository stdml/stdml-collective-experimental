...
class recv_into : public collective_task
{
  ...
  public:
    recv_into(const peer_id id, const session *sess, workspace_state *state)
        : collective_task(state), q(sess->slotbox_->require(id, (*state)->name))
    {
        q->waitQ.put((*state)->recv);
    }

    void poll() override
    {
        if (auto ptr = q->recvQ.try_get(); ptr.has_value()) {
                                 ...
                                 const graph_pair_list &gps, size_t chunk_size)
{
    ...
    auto t = task::par(task::fmap(f, pw), sess->async_rt());
    if (pw.size() > 1) {
